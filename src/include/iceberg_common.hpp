...
static yyjson_val *FindSnapshotByIdInternal(yyjson_val *snapshots,
                                            idx_t target_id) {
  size_t idx, max;
  yyjson_val *snapshot;
...
static yyjson_val *FindSnapshotByIdTimestampInternal(yyjson_val *snapshots,
...
static string ReadMetaData(string &path, FileSystem &fs) {
  ...
  auto metadata_file_path =
      fs.JoinPath(meta_path, "v" + to_string(table_version) + ".metadata.json");
...
static IceBergSnapshot GetSnapshotById(string &path, FileSystem &fs,
                                       idx_t snapshot_id) {
  auto metadata_json = ReadMetaData(path, fs);
  auto doc = yyjson_read(metadata_json.c_str(), metadata_json.size(), 0);
  auto root = yyjson_doc_get_root(doc);
...
static IceBergSnapshot GetSnapshotByTimestamp(string &path, FileSystem &fs,
                                              timestamp_t timestamp) {
  auto metadata_json = ReadMetaData(path, fs);
  auto doc = yyjson_read(metadata_json.c_str(), metadata_json.size(), 0);
  auto root = yyjson_doc_get_root(doc);
