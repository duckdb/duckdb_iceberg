...
static unique_ptr<TableRef> MakeScanExpression(const string &iceberg_path, FileSystem &fs,
    ...
    if (delete_file_values.empty()) {
        ...
        left_children.push_back(make_uniq<ComparisonExpression>(
    ...
    // Calculate delete cardinality
    int64_t delete_cardinality = 0;
    for(auto &manifest : data_file_entries) {
    ...
    right_children.push_back(make_uniq<ComparisonExpression>(
        ExpressionType::COMPARE_EQUAL, 
        make_uniq<ColumnRefExpression>("explicit_cardinality"),
...
static unique_ptr<TableRef> IcebergScanBindReplace(ClientContext &context, TableFunctionBindInput &input) {
	...
	if (mode == "list_files") {
		...
		// Calculate data cardinality and delete cardinality within MakeScanExpression
		// Pass predicates if available
		const vector<unique_ptr<Expression>> *predicates = nullptr;
		...
		return MakeScanExpression(iceberg_path, fs, data_entries, delete_file_values, snapshot_to_scan.schema, allow_moved_paths, 
		                          metadata_compression_codec, skip_schema_inference, predicates);
	} else {
