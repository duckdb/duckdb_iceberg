...
static unique_ptr<TableRef> MakeScanExpression(
    ...
    const vector<unique_ptr<Expression>> *predicates,
    const IcebergTable &iceberg_table,
    int64_t data_cardinality,
    int64_t delete_cardinality,
...
static unique_ptr<TableRef> IcebergScanBindReplace(ClientContext &context, TableFunctionBindInput &input) {
    ...
    if (mode == "list_files") {
        ...
        return MakeScanExpression(
            iceberg_path,
            fs,
            data_entries,
            delete_file_values,
            snapshot_to_scan.schema,
            allow_moved_paths,
